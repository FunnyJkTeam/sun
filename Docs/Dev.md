...
1. IDL推荐使用protobuf，后台也可以使用protobuf组合rsp数据
2. 增加注册类api说明
3. 转移API于按版本号区分的子目录，考虑一个api一个文档，文档名与api名相同，如 ./v1/login.md
4. 合并不同login模式为一个接口，设计通用接口
...
+ 请求
  ...
  + ```json
    {
        "accountInfo": {
            "accountType": 1,  // 待验证的账号类型，如：手机号、邮箱等
